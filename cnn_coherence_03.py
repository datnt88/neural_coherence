...
# convolutional layeres concateneted in parallel
convs = []
for filter_length in filter_sizes:
	# add a convolutiaon 1D layer
	conv = Convolution1D(nb_filter=nb_filter, filter_length = filter_length, border_mode='valid', 
            activation='relu', subsample_length=1)(x)

	# add max pooling layers
	z = MaxPooling1D(pool_length=pool_length)(conv)
	z = Dropout(dropout_ratio)(z)
	z = Flatten()(z)
	convs.append(z)

if len(filter_sizes)>1:
    out_x = merge(convs, mode='concat')
else:
    out_x = convs[0]


out_x = Dropout(dropout_ratio)(out_x)
out_x = Dense(hidden_size, activation='relu')(out_x)
out_x = Dropout(dropout_ratio)(out_x)

# add latent coherence score
out_x = Dense(1, activation='linear')(out_x)
